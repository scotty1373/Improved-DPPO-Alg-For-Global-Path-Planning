        ...
        for idx_barr in range(barr_num):
            ...
            for row_offset in range(self.size[0]):
                for col_offset in range(self.size[1]):
                    ...
                    if dist <= barr_list['radius'][idx_barr] * ratio:
                        heat_mat[row_offset, col_offset] = self.barr_reward * 1 / 2 * (
                                    1 / barr_list['radius'][idx_barr] * (1 - ratio))
                    elif barr_list['radius'][idx_barr] < dist <= barr_list['radius'][idx_barr] * 1.5:
                        heat_mat[row_offset, col_offset] = self.barr_reward * 1 / 2 * (
                                    1 / dist - 1 / barr_list['radius'][idx_barr] * ratio)
    ...
    def reach_rewardCal(self, reachinfo):
        ...
        for row_offset in range(self.size[0]):
            for col_offset in range(self.size[1]):
                ...
                _mat[row_offset, col_offset] = 1/2 * 1 * np.sqrt(dist)
